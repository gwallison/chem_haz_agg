...
STEPS = [
    ...
    Step(
        id="run-echa5",
        name="ECHA 5 — Self-Classified Scrape",
        category="AUTO",
        stage="Automated Collection",
        description="Scrape ECHA Industry (self-classified) data for every substance with a resolved link. Resumable.",
        cmd=["python", "src/01_collection/ECHA_self_classified_scraper.py"],
        use_scrape_env=True,
        dependencies=["run-echa3"],
    ),
    Step(
        id="run-echa5-consolidate",
        name="ECHA 5 — Self-Classified Consolidate",
        category="AUTO",
        stage="Automated Collection",
        description="Summarize scraped Industry data into per-CASRN GHS_H_Codes (feeds Tier 2 only).",
        cmd=["python", "src/01_collection/ECHA_self_classified_consolidator.py"],
        dependencies=["run-echa5"],
    ),
    Step(
        id="run-atsdr",
        name="ATSDR",
    ...
    Step(
        ...
        dependencies=["run-pubchem", "run-echa2", "run-echa5-consolidate", "run-japan", "run-australia", "run-epa4"],
    ),
    Step(
