...
def make_prompt(cas,name,tier_lst,lists_of_concern,
    ...
    ## Description of the data below:
        ...
        "is_IRIS_chem" indicates that the chemical has been formally evaluated by the EPA's Integrated Risk Information System (IRIS),
        which represents one of the most rigorous and comprehensive toxicological assessments available. A True value here is a
        strong indicator that the chemical is well characterized from a hazard standpoint.

        "is_PPRTV_chem" indicates that the chemical has a Provisional Peer-Reviewed Toxicity Value (PPRTV) from the EPA.
        This is similar to an IRIS assessment but is considered a provisional (less exhaustive) evaluation. A True value still
        indicates a meaningful level of toxicological study, though somewhat less authoritative than IRIS.

        "num_toxval_records" is the number of records for this chemical in the EPA ToxValDB, a database that aggregates
        toxicity values from many sources. As a reference across the ~3,000 chemicals we index: 25th percentile = 0,
        50th percentile = 2, 75th percentile = 23, 90th percentile = 84, 95th percentile = 137, max = 925.
        A higher count suggests the chemical has been studied across multiple programs and is relatively well characterized.
        A value of zero or near-zero indicates sparse toxicological data.
    ...
    is_a_UVCB = {uvcb}
    is_IRIS_chem = {ifs.is_IRIS_chem(cas)}
    is_PPRTV_chem = {ifs.is_PPRTV_chem(cas)}
    num_toxval_records = {ifs.get_number_toxval_records(cas)}
